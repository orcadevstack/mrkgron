/**
 * LizConMart Smoke + Functional E2E Test Suite
 *
 * Covers:
...
test.describe("Homepage", () => {
  test("renders hero section with key content", async ({ page }) => {
    await page.goto("/");
    await expect(page).toHaveTitle(/LizConMart/i);
...
test.describe("Navigation", () => {
  test("navbar links all resolve (no 404)", async ({ page }) => {
    ...
    for (const route of routes) {
      ...
      expect(title).toContain("LizConMart");
    }
  });
...
test.describe("About Page", () => {
  test("renders mission content", async ({ page }) => {
    ...
    await expect(page.getByText(/about lizconmart/i)).toBeVisible();
...
test.describe("Login Page", () => {
  test("renders sign in form", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /sign in to lizconmart/i })).toBeVisible();
